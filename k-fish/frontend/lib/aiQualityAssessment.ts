...
function calculateGrade(averageScore: number): QualityAssessment['overallGrade'] {
...
function generateFreshnessScore(productName: string): number {
  const baseScore = 60 + Math.random() * 30;
  const variation = (Math.random() - 0.5) * 10;
  return Math.min(100, Math.max(0, baseScore + variation));
}

function generateColorScore(): number {
  const baseScore = 65 + Math.random() * 25;
  const variation = (Math.random() - 0.5) * 8;
  return Math.min(100, Math.max(0, baseScore + variation));
}

function generateSizeScore(): number {
  const baseScore = 70 + Math.random() * 20;
  const variation = (Math.random() - 0.5) * 5;
  return Math.min(100, Math.max(0, baseScore + variation));
}

function generateDamageScore(): number {
  const baseScore = 75 + Math.random() * 20;
  const variation = (Math.random() - 0.5) * 10;
  return Math.min(100, Math.max(0, baseScore + variation));
}

function getFreshnessDetails(score: number): string {
...
function getColorDetails(score: number): string {
...
function getSizeDetails(score: number): string {
...
function getDamageDetails(score: number): string {
...
export function getGradeColor(grade: QualityAssessment['overallGrade']): string {
  switch (grade) {
    case 'A+':
      return 'bg-green-500';
    case 'A':
...
export function getGradeTextColor(grade: QualityAssessment['overallGrade']): string {
  switch (grade) {
    case 'A+':
