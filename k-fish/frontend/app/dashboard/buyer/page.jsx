...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    ...
    const pickupData = myWins.map(auction => {
      ...
      return {
        ...
        finalPrice,
        auctionEndDate,
        pickupStatus) > 0.4 ? 'picked_up' ,
        pickupLocation,
        pickupDeadline) + 2 * 24 * 60 * 60 * 1000).toISOString(),
        verificationCode) * 100000).toString().padStart(5, '0')}`
      };
    });
    ...
    const mockPickupData = [
      {
        ...
        weight,
        finalPrice,
        auctionEndDate) - 2 * 24 * 60 * 60 * 1000).toISOString(),
        pickupStatus,
        pickupLocation,
        pickupDeadline) + 1 * 24 * 60 * 60 * 1000).toISOString(),
        verificationCode
      },
      {
        id,
      ...
      },
      {
        ...
        pickupStatus,
        pickupLocation,
        pickupDeadline) - 3 * 24 * 60 * 60 * 1000).toISOString(),
        verificationCode
      }
    ];
  ...
  const exportToExcel = () => {
    ...
    const excelData = filteredPickups.map(pickup => ({
      ...
      '중량(kg)',
      '낙찰가격',
      '낙찰일시').toLocaleDateString('ko-KR'),
      '수령상태'=== 'picked_up' ? '수령완료' ,
      '수령장소',
      '수령기한').toLocaleDateString('ko-KR')
    }));
          ...
          <div className="bg-white rounded-lg shadow-md">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600 mt-1">
                현재 진행중인 경매="font-semibold text-blue-600">{liveAuctions.length}건</span>
              </p>
            </div>
            <div className="p-6">
                        ...
                        <div key={pickup.id} className="border-2 border-yellow-200 bg-yellow-50 rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-gray-700 mb-3 bg-white rounded p-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="font-medium">수령기한).toLocaleDateString('ko-KR')}</span>
                            </div>
                          </div>
                        ...
                        <div key={pickup.id} className="border border-gray-200 bg-gray-50 rounded-lg p-4">
                          ...
                          <div className="text-sm text-gray-600">
                            ...
                            <span className="font-medium">
                              {pickup.pickupDate ? new Date(pickup.pickupDate).toLocaleDateString('ko-KR') }
                            </span>
                          </div>
                        </div>
                      ))}
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    조회된 건수="font-semibold">{filteredPickups.length}건</span> | 
                    총 구매액="font-semibold text-blue-600">
            ...
            <div className="p-6">
              {filteredPickups.length > 0 ? (
                        ...
                        return (
                          <tr key={pickup.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="py-3 px-2">
                              ...
                              <div className="text-xs text-gray-500">{pickup.weight}kg | 인증}</div>
                            </td>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">
                    {dateFilter !== 'all' ? '선택한 기간에 구매 내역이 없습니다' }
                  </p>
                  <button
                    onClick={() => window.location.href = '/auction'}
